...
private:
    ...
    bool operator() (const user_info_t &a, const user_info_t &b) const {
      if (a.user.name != b.user.name) {
        ...
        if (a.total_call_duration_s != b.total_call_duration_s) {
          ...
          for (int i = 0; i < a.user.number.size(); ++i) {
            if (i >= b.user.number.size()) {
              return false;
            }
            char fi = a.user.number[i];
            char se = b.user.number[i];
            if (fi < se) {
              return true;
            }
            if (fi > se)
          ...
          return a.user.number.size() < b.user.number.size();
        }
      }
    }
  };

  struct cmp_search_numbers {
    bool operator() (const user_info_t &a, const user_info_t &b) const {
      if (a.total_call_duration_s!= b.total_call_duration_s) {
        return a.total_call_duration_s > b.total_call_duration_s;
      }
  ...
  std::set <user_info_t, cmp_search_names> users;
  std::unordered_map <std::string, std::set<user_info_t, cmp_search_numbers>> numbers;
  std::vector <call_t> history;
  std::map <std::string, bool> was;
  std::map <std::string, user_info_t> find_user;
};